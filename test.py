	...
	for appliance_index, appliance_name in enumerate(appliances):
		...
		for home_id in range(no_of_homes):
			...
			for b in range(num_batches):
				batch_mains = home_mains.iloc[b*batch_size : (b+1)*batch_size]
				processed_batch_mains = mains_preprocessing([batch_mains], sequence_length)
				processed_batch_mains = (processed_batch_mains - mains_mean)/mains_std
				a =time.time()
				batch_prediction = predict(model, processed_batch_mains, cuda, batch_size)
				b=time.time()
				total_time+=b-a
				batch_prediction = batch_prediction * app_std + app_mean
				batch_prediction = batch_prediction.flatten()
				batch_prediction = np.where(batch_prediction>0, batch_prediction,0)
				df = pd.DataFrame({appliance_name: batch_prediction})
				df.index = batch_mains.index
				home_prediction.append(df)
			home_prediction = pd.concat(home_prediction,axis=0)
			appliance_prediction.append(home_prediction)
